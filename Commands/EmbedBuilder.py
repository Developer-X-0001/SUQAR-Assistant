...
class ImportView(discord.ui.View):
    def __init__(self, base_view):
        ...
        self.return_to = None
        # the function to run after getting json data from any of the 3 ways
    ...
    async def mystbin_button(self, interaction, button):
        ...
        try:
            ...
            # can be improved, see BaseView.export_data()
            url = 'https://api.mystb.in/paste/{}'.format(paste_id)
            async with self.base_view.session.get(url) as resp:
                assert resp.status == 200, 'Failed to fetch paste from MystBin.'
                json_data = await resp.json(content_type=None)
                # ['content'] is a str
                data = json.loads(json_data['files'][0]['content'])
...
class SelectView(discord.ui.View):
    ...
    def update_options(self, options):
        ...
        if len(options) > n:
            ...
            self.options_list = [options[i:i+n]
    ...
    async def left_button(self, interaction, button):
        ...
        self.right_button.disabled = False
        self.what_button.label = 'Page {}/{}'.format(
            self.page_index+1, len(self.options_list))
...
class SendView(discord.ui.View):
    ...
    async def channel_button(self, interaction, button):
        ...
        options = [
            ...
            # anti-abuse, user needs send msg perms
    ...
    async def webhook_button(self, interaction, button):
        ...
        try:
            webhook = discord.Webhook.from_url(
                webhook_url, session=self.base_view.session)
        ...
        try:
            ...
            if interaction.guild:
                ...
                assert channel.permissions_for(
                    interaction.user).send_messages, 'You need send_messages permission in the message channel.'
...
class EmbedView(discord.ui.View):
    ...
    # ensures field-related buttons are disabled correctly
    ...
    async def color_button(self, interaction, button):
        ...
        text_input.convert = lambda x: int(
            x) if x.isnumeric() else int(x.lstrip('#'), base=16)
        await self.do(interaction, button, text_input)
    ...
    async def reset_button(self, interaction, button):
        self.embed = discord.Embed.from_dict(
            copy.deepcopy(self.embed_original))
        self.base_view.embeds[self.embed_index] = self.embed
...
class MessageView(discord.ui.View):
    ...
    async def add_embed_button(self, interaction, button):
        embed = discord.Embed(title='New embed {}'.format(
            len(self.base_view.embeds)+1))
    ...
    async def import_button(self, interaction, button):
        async def return_callback(interaction, data):
            ...
            # convert embeds from json to objects
    ...
    def set_import(self, return_callback, return_to):
        ...
        if return_to == 'message':  # hide import from message button if its not message json
            if not view.has_message_button:
                ...
                view.remove_item(view.back_button)
...
# -------------------------------------- EmbedBuilder End -------------------------------------- EmbedBuilder End --------------------------------------
...
class MessageMaker(commands.Cog):
    ...
    @app_commands.checks.has_role(1059612373423497316)
