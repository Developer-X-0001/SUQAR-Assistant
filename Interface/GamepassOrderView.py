...
class GamepassOrderExtrasButtons(View):
    def __init__(self):
    ...
    async def proceed(self, interaction: discord.Interaction, button: Button):
        ...
        data = orders_database.execute("SELECT order_no, user_id, gamepass_amount, gamepass_type, gamepass_payment FROM GamepassOrders WHERE order_no = ?", (order_no,)).fetchone()
...
class GamepassOrderModal(Modal, title="Gamepass Order Form"):
    def __init__(self):
        super().__init__(timeout=None)

    gamepass_amount = TextInput(
        label="How many gamepasses do you want?",
        style=TextStyle.short,
        required=True
    )
    gamepass_type = TextInput(
        label="What kind of gamepasses do you want?",
        style=TextStyle.short,
        required=True
    )
    gamepass_price = TextInput(
        label="Payment?",
        style=TextStyle.short,
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        orders_database.execute(
            '''
                INSERT INTO GamepassOrders VALUES (?, ?, ?, ?, ?)
            ''',
            (
                order_no,
                interaction.user.id,
                self.gamepass_amount.value,
                self.gamepass_type.value,
                self.gamepass_price.value,
            )
        ).connection.commit()
        ...
        await interaction.response.edit_message(embed=embed, view=GamepassOrderExtrasButtons())